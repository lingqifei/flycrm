...
class FinIncomeRecord extends Action{	
	...
	public function __construct() {
		...
		$this->type=_instance('Action/erp/FinIncomeType');
	...
	public function fin_income_record(){
		...
		$countSql  = "select r.* from fin_income_record as r left join fin_income_type as t on r.type_id=t.id where {$where_str}";
		$totalCount	= $this->C($this->cacheDir)->countRecords($countSql);	//计算记录数
		$beginRecord = ($pageNum-1)*$pageSize;
		
		$moneySql  ="select sum(money) as total_money from fin_income_record as r left join fin_income_type as t on r.type_id=t.id 
						where {$where_str}";
		$moneyRs	= $this->C($this->cacheDir)->findOne($moneySql);

		$sql		 = "select r.*,t.name as type_name from fin_income_record as r left join fin_income_type as t on r.type_id=t.id
	...
	public function fin_income_record_json(){
	...
	public function fin_income_record_show(){
		...
		$smarty->display('erp/fin_income_record_show.html');	
	}		

	public function fin_income_record_add(){
		if(empty($_POST)){
			$type_id=$this->type->fin_income_type_select_tree('type_id');
			$bank=$this->bank->fin_bank_accoun_select();
			$smarty = $this->setSmarty();
			$smarty->assign(array("type_id"=>$type_id,"bank"=>$bank));
			$smarty->display('erp/fin_income_record_add.html');	
		}else{
			$bank_id=$this->_REQUEST("bank_id");
			$money=$this->_REQUEST("money");
			$into_data=array(
			...
			//插入记录
			$record_id=$this->C($this->cacheDir)->insert('fin_income_record',$into_data);
			//增加财务流水
			$this->flow->fin_flow_record_add('rece',$money,$bank_id,$record_id,'fin_income_record');
	...
	public function fin_income_record_del(){
		...
		foreach($record_arr as $id){
			$sql ="select * from fin_income_record where record_id='$id'";
			...
			//删除记录
			$this->C($this->cacheDir)->delete('fin_income_record',"record_id='$id'");
			//增加财务流水
			$this->flow->fin_flow_record_add('pay',$money,$bank_id,$id,'fin_income_record');
		}
		$this->L("Common")->ajax_json_success("操作成功");	
	}		
}//end class
