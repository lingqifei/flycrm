...
class Message extends Action{
	...
	public function message(){
		...
		//**************************************************************************
		$countSql   = "select u.id from fly_sys_message as u where $where_str";
		$totalCount  = $this->C($this->cacheDir)->countRecords($countSql);	//计算记录数
		$beginRecord = ($pageNum-1)*$pageSize;
		$sql		 = "select u.* from fly_sys_message as u where $where_str order by u.id desc limit $beginRecord,$pageSize";
	...
	public function message_show(){
		...
		$smarty->display('sysmanage/message_show.html');	
	}	
	
	//定制调用显示公告
	public function message_cron(){
		$sql ="select u.* from fly_sys_message as u where flag='-1' and u.owner_user_id = '".SYS_USER_ID."' and (remind_time='0000-00-00 00:00:00' or now()>remind_time) order by create_time desc";
		$list= $this->C($this->cacheDir)->findAll($sql);
		foreach($list as $key=>$row){
	...
	public function message_flag($key=null){
		$data=array(
			"-1"=>array(
				 		'flag_name'=>'未查看',
				 		'color'=>'#FAD733',
				 		'flag_name_html'=>'<span class="label label-info">未查看<span>',
					),
			"1"=>array(
				 		'flag_name'=>'已查看',
						'color'=>'#23B7E5',
				 		'flag_name_html'=>'<span class="label label-default">已查看<span>',
					)
		);
