...
class SalContract extends Action
{
    ...
    public function sal_contract()
    {
        ...
        //开始时间
        if (!empty($start_date)) {
            $date_range = $this->comm->date_range('-1', $start_date);
            $where_str .= " and s.start_date>'$date_range'";
        }
        //到期时间
        if (!empty($end_date)) {
            $date_range = $this->comm->date_range('-1', $end_date);
...
//                    ),
    ...
    public function sal_contract_deliver_status($key = null)
    {
        $data = array(
            "-1" => array(
                'status_name' => '无需交付',
                'status_name_html' => '<span class="label label-info">无需交付<span>',
                'status_operation' => array(
                    '0' => array(
                        'act' => 'deliver_open',
                        'color' => '#23B7E5',
    ...
    //开启交付
    public function sal_contract_deliver_open()
    {
        $contract_id = $this->_REQUEST('contract_id');
        $upt_data = array(
            'deliver_status' => '1',
        );
        $this->C($this->cacheDir)->modify('sal_contract', $upt_data, "contract_id='$contract_id'", true);
        $this->L("Common")->ajax_json_success("操作成功");
    }

    //关闭交付
    public function sal_contract_deliver_close()
    {
        $contract_id = $this->_REQUEST('contract_id');
        $upt_data = array(
            'deliver_status' => '-1',
