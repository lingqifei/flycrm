 ...
 */	
class HrmStaffTalk extends Action{	
	...
	public function hrm_staff_talk(){
		//**获得传送来的数据作分页处理
		...
		//**************************************************************************
		$countSql   = "select c.* from hrm_staff_talk as c left join  hrm_staff as s on c.staff_id=s.id  where $where_str";
		$totalCount	 = $this->C($this->cacheDir)->countRecords($countSql);	//计算记录数
		$beginRecord= ($pageNum-1)*$pageSize;//计算开始行数
		$sql		 = "select c.* from hrm_staff_talk as c left join  hrm_staff as s on c.staff_id=s.id  where $where_str $order_by limit $beginRecord,$pageSize";
	...
	public function hrm_staff_talk_json(){
		$assArr  = $this->hrm_staff_talk();
		echo json_encode($assArr);
	}	
	//客户列表显示
	public function hrm_staff_talk_show(){
		$smarty = $this->setSmarty();
		$smarty->display('hrm/hrm_staff_talk_show.html');	
	}	

	//客户增加
	public function hrm_staff_talk_add(){
	    $staff_id	= $this->_REQUEST("staff_id");
		if(empty($_POST)){
            $staff_list=$this->staff->hrm_staff_list();
            $smarty = $this->setSmarty();
            $smarty->assign(array("staff_id"=>$staff_id,"staff_list"=>$staff_list));
			$smarty->display('hrm/hrm_staff_talk_add.html');	
		}else{
            $into_data=array(
                'staff_id'=>$this->_REQUEST("staff_id"),
            ...
            $this->C($this->cacheDir)->insert('hrm_staff_talk',$into_data);
            $this->L("Common")->ajax_json_success("操作成功");
        }
	}
	public function hrm_staff_talk_modify(){
		$id	= $this->_REQUEST("id");
		if(empty($_POST)){
			$sql 	= "select * from hrm_staff_talk where id='$id'";
			$one 	= $this->C($this->cacheDir)->findOne($sql);
			$smarty = $this->setSmarty();
			$smarty->assign(array("one"=>$one));
			$smarty->display('hrm/hrm_staff_talk_modify.html');	
			...
			$this->C($this->cacheDir)->modify('hrm_staff_talk',$into_data,"id='$id'");
			$this->L("Common")->ajax_json_success("操作成功");			
		}
	}
	//删除
	public function hrm_staff_talk_del(){
		$id = $this->_REQUEST("id");
		$sql  = "delete from hrm_staff_talk where id in ($id)";
		$this->C($this->cacheDir)->update($sql);	
		$this->L("Common")->ajax_json_success("操作成功");	
	}	
	//查询一条记录
	public function hrm_staff_talk_get_one($id=""){
		if($id){
			$sql = "select * from hrm_staff_talk where id='$id' ";
			$one = $this->C($this->cacheDir)->findOne($sql);
			return $one;
