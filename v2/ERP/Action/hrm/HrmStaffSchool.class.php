 ...
 */	
class HrmStaffSchool extends Action{	
	...
	public function hrm_staff_school(){
		//**获得传送来的数据作分页处理
		...
		//**************************************************************************
		$countSql   = "select c.* from hrm_staff_school as c where $where_str";
		$totalCount	 = $this->C($this->cacheDir)->countRecords($countSql);	//计算记录数
		$beginRecord= ($pageNum-1)*$pageSize;//计算开始行数
		$sql		 = "select c.* from hrm_staff_school as c where $where_str $order_by limit $beginRecord,$pageSize";	
	...
	public function hrm_staff_school_json(){
		$assArr  = $this->hrm_staff_school();
		echo json_encode($assArr);
	}	
	//客户列表显示
	public function hrm_staff_school_show(){
		$smarty = $this->setSmarty();
		$smarty->display('hrm/hrm_staff_school_show.html');	
	}	

	//客户增加
	public function hrm_staff_school_add(){
	    $staff_id	= $this->_REQUEST("staff_id");
		if(empty($_POST)){
			$smarty = $this->setSmarty();
			$smarty->assign(array("staff_id"=>$staff_id));
			$smarty->display('hrm/hrm_staff_school_add.html');	
		}else{
            $into_data=array(
                'staff_id'=>$this->_REQUEST("staff_id"),
                'name'=>$this->_REQUEST("name"),
            ...
            $this->C($this->cacheDir)->insert('hrm_staff_school',$into_data);
            $this->L("Common")->ajax_json_success("操作成功");
        }
	}
	public function hrm_staff_school_modify(){
		$id	= $this->_REQUEST("id");
		if(empty($_POST)){
			$sql 	= "select * from hrm_staff_school where id='$id'";
			$one 	= $this->C($this->cacheDir)->findOne($sql);
			$smarty = $this->setSmarty();
			$smarty->assign(array("one"=>$one));
			$smarty->display('hrm/hrm_staff_school_modify.html');	
			...
			$this->C($this->cacheDir)->modify('hrm_staff_school',$into_data,"id='$id'");
			$this->L("Common")->ajax_json_success("操作成功");			
		}
	}
	//删除
	public function hrm_staff_school_del(){
		$id = $this->_REQUEST("id");
		$sql  = "delete from hrm_staff_school where id in ($id)";
		$this->C($this->cacheDir)->update($sql);	
		$this->L("Common")->ajax_json_success("操作成功");	
	}	
	//查询一条记录
	public function hrm_staff_school_get_one($id=""){
		if($id){
			$sql = "select * from hrm_staff_school where id='$id' ";
			$one = $this->C($this->cacheDir)->findOne($sql);
			return $one;
