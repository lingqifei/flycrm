...
class PosContractFile extends Action
{
    ...
    public function pos_contract_file()
    {
        //**获得传送来的数据作分页处理
        ...
        $countSql = "select f.* from pos_contract_file as f where $where_str";
        $totalCount = $this->C($this->cacheDir)->countRecords($countSql);

        $beginRecord = ($pageNum - 1) * $pageSize;//计算开始行数

        $sql = "select f.* from pos_contract_file as f where  $where_str $order_by limit $beginRecord,$pageSize";
    ...
    public function pos_contract_file_json()
    {
        $assArr = $this->pos_contract_file();
        echo json_encode($assArr);
    }

    //合同显示
    public function pos_contract_file_show()
    {
    ...
    public function pos_contract_file_add()
    {
        $contract_id = $this->_REQUEST("contract_id");
        if (empty($_POST)) {
            ...
            $smarty->display('erp/pos_contract_file_add.html');
        } else {
			$contract_id=$this->_REQUEST("contract_id");
			...
			if(!empty($imglistname)){
				foreach($imglistname as $row){
					$sql = "insert into pos_contract_file(contract_id,name,filepath) values('$contract_id','$name','$row')";
    ...
    //删除
    public function pos_contract_file_del()
    {
        $id = $this->_REQUEST("id");
		$this->C($this->cacheDir)->delete('pos_contract_file', "id in ($id)");
		$this->L("Common")->ajax_json_success("操作成功");
    }
}
