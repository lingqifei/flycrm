 ...
 */	
class StockIntoList extends Action{	
	...
	public function __construct() {
		...
		$this->stock_into=_instance('Action/erp/StockInto');
	}	
	//库存清单商品SKU
	public function stock_into_list(){
		//**获得传送来的数据作分页处理
		...
		$countSql  = "select g.goods_name,s.* from stock_into_list as s 
						left join fly_goods as g on g.goods_id=s.goods_id 
						left join stock_into as i on i.into_id=s.into_id 
						where $where_str";
		$totalCount	= $this->C($this->cacheDir)->countRecords($countSql);	
		$beginRecord= ($pageNum-1)*$pageSize;//计算开始行数
		$sql ="select g.goods_name,g.category_id,s.*,i.title from stock_into_list as s 
				left join fly_goods as g on g.goods_id=s.goods_id 
				left join stock_into as i on i.into_id=s.into_id
	...
	public function stock_into_list_json(){
		$assArr  = $this->stock_into_list();
		echo json_encode($assArr);
	}	
	public function stock_into_list_show(){
		$assArr  		= $this->stock_into_list();
		$smarty  		= $this->setSmarty();
		$smarty->assign($assArr);
		$smarty->display('erp/stock_into_list_show.html');	
	}
