...
class PosContractList extends Action
{
    ...
    {
        $sql = "select goods_name,sku_name,num,cost_price,goods_money,into_num,create_time from pos_contract_list;";
        $list = $this->C($this->cacheDir)->findAll($sql);
        $title_cel = array('商品名称', '规格名称', '采购数量', '采购价格', '采购金额', '入库数量', '创建时间');
        export_to_cvs('采购明细_' . time() . '.csv', $title_cel, $list);
    }
}
?>