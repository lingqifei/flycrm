<?php /* Smarty version 2.6.26, created on 2019-11-20 11:14:13
         compiled from erp/stock_out_show.html */ ?>
<!DOCTYPE html>
<html>
    ...
    <div class="col-sm-12">
      <div class="ibox float-e-margins">
        <div class="ibox-title">
          <h5><i class="fa fa-home"></i>出库单列表</h5>
          <div class="ibox-tools"><a href="javascript:void(0);" class="btn btn-xs btn-default btn-help-detail" data-type="stock_out"> <i class="fa fa-question-circle"> 操作说明</i></a> </div>
        </div>
          ...
          <table class="table table-hover sorttable 07fly-table" width="100%">
            <thead>
              <tr>
                ...
                <th width="100"><span>出库数量</span></th>
                <th width="120"><span>出库人员/时间</span></th>
                <th width="120"><span>出库类型</span></th>
                <th><span>备注</span></th>
...
<% for(var i=0;i<list.length;i++) { %>
	<tr>
		<td><input name="out_id" class="checkboxCtrlId" value="<%=list[i]['out_id']%>" type="checkbox"></td>
		<td><%=list[i]['title']%></td>
		...
		<td><%=list[i]['number']%></td>
		<td>
			<%=list[i]['out_user_arr']['name']%>
			<p><small><%=list[i]['out_time']%></small></p>
		</td>
		<td><%=list[i]['out_type']%></td>
		<td><%=list[i]['intro']%></td>		
		<td class="text-center">
			<% for(var j=0;j<list[i]['status_arr']['status_operation'].length;j++) { %>
			<p><a href="javascript:void(0)" class="single_operation" data-id="<%=list[i]['out_id']%>" data-act="<%=list[i]['status_arr']['status_operation'][j]['act']%>"><font color="<%=list[i]['status_arr']['status_operation'][j]['color']%>"><%=list[i]['status_arr']['status_operation'][j]['name']%></font></a></p>
			<% } %>
		</td>
	</tr>
<% } %>
...
$(document).ready(function () {
	...
	$("body").on("click", ".batch_operation", function() {
		...
		out_id_txt=chk_value.join(",");
		if(batch_act=="export"){
		  	act_url="<?php echo @ACT; ?>
/erp/StockOut/stock_out_sure/";	
		}else if(batch_act=="del"){
			act_url="<?php echo @ACT; ?>
/erp/StockOut/stock_out_del/";
		}
    //alert(chk_value.length==0 ?'你还没有选择任何内容！':chk_value); 
		$.ajax({
			type: "POST",
			url: act_url,
			data:{"out_id":out_id_txt},
			dataType:"json",
