<?php /* Smarty version 2.6.26, created on 2019-09-29 18:05:20
...
<% for(var i=0;i<list.length;i++) { %>
	<tr>
		...
		<td><%:=list[i]['deliver_status_arr']['status_name_html']%></td>
		<td align="center">
			...
			<% for(var j=0;j<list[i]['back_status_arr']['status_operation'].length;j++) { %>
				<p><a href="javascript:void(0)" class="single_operation" data-id="<%=list[i]['contract_id']%>" data-act="<%=list[i]['back_status_arr']['status_operation'][j]['act']%>"><font color="<%=list[i]['back_status_arr']['status_operation'][j]['color']%>"><%=list[i]['back_status_arr']['status_operation'][j]['name']%></font></a></p>
			<% } %>
...
/crm/SalContract/sal_contract_json/';
...
/crm/SalContract/sal_contract_modify_recommend/";
		}else if(batch_act=="delete"){
			act_url="<?php echo @ACT; ?>
/crm/SalContract/sal_contract_del/";
		}
...
/crm/SalContract/sal_contract_add/',
				end: function () {
					turnPage(1);//页面加载时初始化分页
				}
			});			
			...
			layer.open({
				...
				area: ['90%', '90%'],
				content: '<?php echo @ACT; ?>
/crm/SalContract/sal_contract_modify/contract_id/'+contract_id+'/',
				end: function () {
					turnPage(1);//页面加载时初始化分页
				}
			});			
...
/crm/SalContractList/sal_contract_list_add/contract_id/'+contract_id+'/',
			...
			return false;	
		}else if(single_act=="fin_rece_add"){
			layer.open({
				type: 2,
				title: '新增回款',
				shadeClose: true,
				...
				area: ['90%', '90%'],
				content: '<?php echo @ACT; ?>
/erp/FinReceRecord/fin_rece_record_add/contract_id/'+contract_id+'/',
				end: function () {
					turnPage(1);//页面加载时初始化分页
				}
			});			
...
/crm/SalContract/sal_contract_detail/contract_id/'+contract_id+'/';
		}else if(single_act=="list_del"){
			act_url="<?php echo @ACT; ?>
/crm/SalContractList/sal_contract_list_del/";
			$.ajax({
				type: "POST",
				url: act_url,
				data:{"contract_id":contract_id},
		...
		}else if(single_act=="deliver_open"){
			act_url="<?php echo @ACT; ?>
/crm/SalContract/sal_contract_deliver_open/";
			$.ajax({
				type: "POST",
				url: act_url,
				data:{"contract_id":contract_id},
		...
		}else if(single_act=="deliver_close"){
			act_url="<?php echo @ACT; ?>
/crm/SalContract/sal_contract_deliver_close/";
			$.ajax({
				type: "POST",
				url: act_url,
				data:{"contract_id":contract_id},
