...
class Cache
{
    ...
    */
    ...
    {
        $database = _instance('Database', '', 1);
        $result = $database->insert($table, $arrayDataValue, $debug); //执行更新的SQL语句
        $this->upt_cache_dir($result);
        return $result; //返回执行结果，影响记录数或新增记录ID 或false
    }
    ...
    {
        $database = _instance('Database', '', 1);
        $result = $database->replace($table, $arrayDataValue, $debug); //执行更新的SQL语句
        $this->upt_cache_dir($result);
        return $result; //返回执行结果，影响记录数或新增记录ID 或false
    }
    ...
    {
        $database = _instance('Database', '', 1);
        $result = $database->delete($table, $where, $debug); //执行更新的SQL语句
        $this->upt_cache_dir($result);
        return $result; //返回执行结果，影响记录数或新增记录ID 或false
    }
