...
var APP_DOMAIN = 'http://' + host + '/index.php';
//为true输出日志
var debug = true;
var PAY_DOMAIN = ''
 ...
 */
function checklogin() {
    request("/wap/Auth/check_login/", {}, '',false,false);
...
function request(method, parm, callback, showwait, errcallback, shownetmsg) {
    ...
    mui.ajax(APP_DOMAIN + method, {
        ...
        error: function (xhr, type, errorThrown) { //失败，打一下失败的类型，主要用于调试和用户体验
            ...
            if (showwait) appUI.closeWaiting();
            ...
            if (type == 'timeout' || type == 'abort') {
                ...
                if (shownetmsg) mui.toast("请求超时：请检查网络：" + type)
            } else {

                setRequestMsg("服务器累了");
                if (shownetmsg) mui.toast("服务器累了：" + type)
            }
