 ...
 */	
	public function __construct() {		
		...
		($requestURI ==$appName.'/')? $urlStr='': $urlStr=str_replace (ACT, '', $requestURI );//取得当前index.php之后的字符串						
 ...
 */			
	private function filterParams($urlStr) {
		...
		if(!empty($_SERVER['QUERY_STRING']))$urlStr =str_replace(array('&','='),'/',$urlStr);//如果为查询字符串则过滤字符'&','='
		$urlArr=explode ( '/', $urlStr );
		if($GLOBALS['ReWrite']&&!empty($GLOBALS['htmlExt']))//如果开启重写并且静态后缀不为空则过滤伪静态后缀
