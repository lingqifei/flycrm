 ...
 */	
	public function C($cacheDir='',$cacheTime=0) {	

		return new Cache($cacheDir,$cacheTime);//Framk框架对缓存遵循‘数据更新，缓存更新’原则，但数据更新频率太高的话，用户可以设置缓存时间来延迟更新数据	
	}
...
$info= <<<EOT
<section id="message" alt=2 >
	<h4 class="alert_warning">{$tmp}</h4>
	<h4 class="alert_info">系统将在<span id="totalSecond">3</span>秒钟后自动跳转，如果页面没有跳转请点击<a href='javascript:window.history.go(-1);' style='color:#ff0000'>【这里】</a></h4>
</section>
EOT;
}elseif($type==4){
$info= <<<EOT
<section id="message" alt=4 >
	<h4 class="alert_warning">{$tmp}</h4>
	<h4 class="alert_info">系统将在<span id="totalSecond">3</span>秒钟后自动跳转，如果页面没有跳转请点击<a href='javascript:window.history.go(-1);' style='color:#ff0000'>【这里】</a></h4>
</section>
EOT;
...
<head>
<meta charset="utf-8"/>
<link rel="stylesheet" href="${app}/View/ui/black/css/layout.css" type="text/css" media="screen" />
...
<script language="javascript" type="text/javascript">   
	...
	var alt = document.getElementById('message').getAttribute("alt"); 
