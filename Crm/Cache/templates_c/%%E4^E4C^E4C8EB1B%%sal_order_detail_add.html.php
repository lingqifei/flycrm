...
/ord_number/<?php echo $this->_tpl_vars['one']['ord_number']; ?>
/" method="post" class="pageForm required-validate" onsubmit="return validateCallback(this, navTabAjaxDone)">
<input type="hidden" name="order_number" value="<?php echo $this->_tpl_vars['one']['ord_number']; ?>
" />
...
/CstLinkman/cst_linkman_select/cusID/{cusID}/" warn="请选择客户名称" lookupGroup="linkman" readonly="readonly"/>
        </p>
        <p>
          <label>订单主题：</label>
...
" name="our.name" type="text" postField="keyword" suggestFields="title" 
					suggestUrl="<?php echo @ACT; ?>
/User/cst_chance_select/cusID/{cusID}/" warn="请选择客户名称" lookupGroup="our" readonly="readonly"/>
        </p>
        <p>
          <label>建档时间：</label>
...
?>
              <tr class="unitBox">
                <td><input  name="items.name[]" type="text" postField="keyword" suggestFields="items.name[],pro_number[],model[],norm[],price[]" fieldClass="required" suggestUrl="<?php echo @ACT; ?>
/Product/lookup_search/" lookupGroup="items" class="required" value="<?php echo $this->_tpl_vars['v']['name']; ?>
" size="12"/><a class="btnLook" href="<?php echo @ACT; ?>
/Product/lookup_search/" lookupGroup="items" suggestFields="items.name[],pro_number[],model[],norm[],price[]" suffix="[]" >查找带回</a></td>
