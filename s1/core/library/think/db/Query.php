...
class Query
{
     ...
     * @param string $sum 需要统计字段生成后缀
     * @param bool $left_alias  左表别名
     * @param string $filed_alias  计字段生成后缀，默认为_sum
     * @return $this
     */
    public function withSum($relation, $subQuery = true, $sum='num',$left_alias=false,$filed_alias=false)
    {
        ...
        if (!$subQuery) {
            ...
            foreach ($relations as $key => $relation) {
                ...
                if($filed_alias){
                    $this->field([$count => Loader::parseName($relation) . '_'.$filed_alias]);
                }else{
