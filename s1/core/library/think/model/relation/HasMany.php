...
class HasMany extends Relation
{
    ...
    public function getRelationSumQuery($closure, $sum,$left_alias)
    {
        ...
        $localKey = $this->localKey ?: $this->parent->getPk();
        return $this->query->whereExp($this->foreignKey, '=' . $table . '.' . $localKey)->fetchSql()->sum($sum);

        //return $this->query->where([$this->foreignKey => [ 'exp', '=' . $table . '.' . $this->parent->getPk(),  ], ])->fetchSql()->sum($sum);
    }
