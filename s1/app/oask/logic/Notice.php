...
class Notice extends OaskBase
{
	 ...
	 * @return
	 */
	public function getNoticeList($where = [], $field = true, $order = 'a.create_time desc', $paginate = DB_LIST_ROWS)
	{
		$this->modelNotice->alias('a');
		$list = $this->modelNotice->getList($where, $field, $order, $paginate);
	...
	public function noticeAdd($data = [])
	{
		...
		$validate_result = $this->validateNotice->scene('add')->check($data);
		if (!$validate_result) {
		...
		$url = url('show');
		$result && action_log('新增', '新增公告：' . $data['name']);

		return $result ? [RESULT_SUCCESS, '添加成功', $url] : [RESULT_ERROR, $this->modelNotice->getError()];
	}
	...
	public function noticeEdit($data = [])
	{
		...
		$result && action_log('编辑', '编辑公告，name：' . $data['name']);

		return $result ? [RESULT_SUCCESS, '编辑成功', $url] : [RESULT_ERROR, $this->modelNotice->getError()];
	}
	...
	public function noticeDel($where = [])
	{
		...
		$result = $this->modelNotice->deleteInfo($where, true);

		$result && action_log('删除', '删除公告，where：' . http_build_query($where));

		return $result ? [RESULT_SUCCESS, '删除成功'] : [RESULT_ERROR, $this->modelNotice->getError()];
	}
	...
	{
		$info= $this->modelNotice->getInfo($where, $field);
