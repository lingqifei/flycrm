 ...
 * ==============================================
 * Assetsor: kfrs <goodkfrs@QQ.com> 574249366
 * Date: 2019-10-3
 */
...
class Assets extends OaskBase
{
     ...
     * @return
     */
    public function getAssetsList($where = [], $field = true, $order = 'update_time desc', $paginate = DB_LIST_ROWS)
    {
        $list = $this->modelAssets->getList($where, $field, $order, $paginate);

        foreach ($list as &$row) {
            $row['type_name']=$this->modelAssetsType->getValue(['id'=>$row['type_id']],'name');
        }
        return $list;
    }
     ...
     * @return array
     */
    public function assetsAdd($data = [])
    {

        $validate_result = $this->validateAssets->scene('add')->check($data);
        if (!$validate_result) {
            return [RESULT_ERROR, $this->validateAssets->getError()];
        }
		$data['create_user_id'] = SYS_USER_ID;
		$result = $this->modelAssets->setInfo($data);

        $url = url('show');
        $result && action_log('新增', '新增资产列表：' . $data['name']);
        return $result ? [RESULT_SUCCESS, '添加成功', $url] : [RESULT_ERROR, $this->modelAssets->getError()];
    }
     ...
     * @return array
     */
    public function assetsEdit($data = [])
    {

        $validate_result = $this->validateAssets->scene('edit')->check($data);
        if (!$validate_result) {
            return [RESULT_ERROR, $this->validateAssets->getError()];
        }
        ...
        $result && action_log('编辑', '编辑资产列表，name：' . $data['name']);
        $url = url('show');
        return $result ? [RESULT_SUCCESS, '编辑成功', $url] : [RESULT_ERROR, $this->modelAssets->getError()];
    }
     ...
     * @return array
     */
    public function assetsDel($data = [])
    {
        if(!empty($data['id'])){
            $where['id'] = ['=',$data['id']];
        }else{
        ...
        $result = $this->modelAssets->deleteInfo($where, true);
        $result && action_log('删除', '删除资产列表，where：' . http_build_query($where));
        return $result ? [RESULT_SUCCESS, '删除成功'] : [RESULT_ERROR, $this->modelAssets->getError()];
    }
     ...
     * @return
     */
    public function getAssetsInfo($where = [], $field = true)
    {
        $info= $this->modelAssets->getInfo($where, $field);
        $info['use_user_name'] = $this->logicSysUser->getValue(['id'=>$info['use_user_id']],'realname');
     ...
     * @return
     */
    public function getAssetsDetail($data = [])
    {
        $rtnArray=[];
    ...
    public function getWhere($data = [])
    {
        ...
        $where = [];
        //关键字查
        !empty($data['keywords']) && $where['name|assets_no|address|remark'] = ['like', '%' . $data['keywords'] . '%'];
	...
	{
		return $this->modelAssets->status($key);
	}
